...
@pytest.mark.asyncio
async def test_get_or_create_chat_updates_default_name_when_user_title_arrives(
    tmp_path: Path,
):
    repo = JsonChatRepository(tmp_path / "chats.json")
    ...
    await manager.create_chat(original)
    ...
    assert existing.name == "帮我找 AI 产品经理"
    persisted = await repo.get_chat("chat-1")
    assert persisted is not None
    assert persisted.name == "帮我找 AI 产品经理"
