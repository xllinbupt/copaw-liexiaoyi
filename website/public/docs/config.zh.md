...
Talora 的所有配置和数据都存储在**工作目录**中。本页说明：

- **目录结构** — 文件都在哪里，各目录的作用
- **环境变量** — 如何用环境变量自定义路径和行为
- **配置文件** — `config.json` 和 `agent.json` 的完整字段说明

从 **v0.1.0** 开始，Talora 支持**多智能体**，配置分为两层：
...
Talora 需要 LLM 提供商才能运行。配置存储在 `$COPAW_SECRET_DIR/providers.json`（默认 `~/.copaw.secret/providers.json`）。
...
> **注意：** 环境变量的值（如第三方 API Key）的有效性需要用户自行保证。Talora 只负责存储和注入，不会校验其正确性。
