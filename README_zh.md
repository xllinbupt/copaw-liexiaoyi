...
<p align="center"><b>给到 HR 的专属猎头 Agent 助手。</b></p>

</div>

面向 HR 与猎头招聘团队的 AI Agent 助手；支持本地部署、招聘流程协同和招聘 Skills 扩展。

> **核心能力：**
>
> **由你掌控** — 候选人资料、招聘知识与流程沉淀完全由你掌控，支持本地或云端部署。无第三方托管，无数据上传。
>
> **Skills 扩展** — 内置招聘场景技能，可自动加载本地自定义 skill。当前已接入升级后的 `resume_search`，支持招聘需求拆解、布尔检索组装、搜索简历并按需拉取详情。
>
> **多智能体协作** — 可创建搜寻、初筛、约面、候选人跟进等不同角色的智能体，协同推进招聘流程。
>
> **多层安全防护** — 工具防护、文件访问控制、技能安全扫描，保障运行安全。
>
> **全域触达** — 控制台、钉钉、飞书、微信等频道统一协作，一个Talora按需连接。
>
> <details>
> <summary><b>你可以用Talora做什么</b></summary>
>
> <br>
>
> - **人才搜寻**：按岗位关键词、城市、年限、学校、公司背景等维度生成搜索计划，桥接为布尔检索请求并检索候选人。
> - **简历查看**：通过内置 `resume_search` skill 按需拉取候选人结构化简历详情。
> - **候选人初筛**：基于 JD 自动提炼匹配亮点、风险项和建议追问。
> - **流程推进**：围绕跟进、约面、反馈与 offer 管理沉淀待办与结论。
> - **招聘知识库**：沉淀职位画像、筛选标准、行业地图与面试模板。
> - **扩展更多**：继续接入你的招聘系统、表格、知识库或内部流程。
>
> </details>
...
无需手动配置 Python，一行命令自动完成安装。脚本会自动下载 uv（Python 包管理器）、创建虚拟环境、安装 Talora 及其依赖（含 Node.js 和前端资源）。注意：部分网络环境或企业权限管控下可能无法使用。
...
>
> Docker 容器内的 `localhost` 指向容器自身，而非宿主机。如果 Ollama（或其他模型服务）运行在宿主机上，可通过以下方式让容器内的 Talora 访问：
>
...
>   agentscope/copaw:latest
> ```
> 然后在 Talora **设置 → 模型** 中，将 Base URL 改为 `http://host.docker.internal:<端口>` — 例如 Ollama 填 `http://host.docker.internal:11434`，LM Studio 填 `http://host.docker.internal:1234/v1`。
>
...
若希望将 Talora 部署在阿里云上，可使用阿里云 ECS 一键部署：打开 [Talora 阿里云 ECS 部署链接](https://computenest.console.aliyun.com/service/instance/create/cn-hangzhou?type=user&ServiceId=service-1ed84201799f40879884) 按页面提示操作即可。详细步骤见 [阿里云开发者社区：Talora 3 分钟部署你的 AI 助理](https://developer.aliyun.com/article/1713682)。

---

### 方式五：使用魔搭创空间

**不想本地安装？** 使用 [魔搭创空间](https://modelscope.cn/studios/fork?target=AgentScope/CoPaw) 一键云端配置。请将创空间设为 **非公开**，否则他人可能操纵你的 Talora。
...
当你从 Releases 下载 Talora macOS 应用时，macOS 可能显示：*"Apple 无法验证 'Talora' 不包含恶意软件"*。这是因为应用未经过公证。你仍然可以通过以下方式打开：

- **右键打开（推荐）**
  右键点击（或 Control + 点击）Talora 应用 → **"打开"** → 在对话框中再次点击 **"打开"**。这会告诉 Gatekeeper 你信任该应用；之后可以像往常一样双击启动。

- **在系统设置中允许**
  如果仍被阻止，进入 **系统设置 → 隐私与安全性**，向下滚动找到类似 *"已阻止 'Talora'，因为无法验证开发者"* 的提示，点击 **"仍要打开"** 或 **"允许"**。
...
若使用**云端大模型 API**（如通义千问、Gemini、OpenAI），在开始对话前必须配置 API Key。未配置有效 Key 前，Talora 无法正常工作。详情请参考[官方文档](https://copaw.agentscope.io/docs/models)。
...
Talora 可在本机完全本地运行大模型，无需 API Key 或云端服务。详情请见[官方文档](https://copaw.agentscope.io/docs/models)
...
| 主题                                                      | 说明                                 |
| --------------------------------------------------------- | ------------------------------------ |
| [项目介绍](https://copaw.agentscope.io/docs/intro)        | Talora 是什么、怎么用                 |
...
在 GitHub 上 Star Talora，第一时间收到新版本发布通知。
...
Talora 在开放协作中持续演进，欢迎各种形式的参与！请参考上方 [路线图](#路线图)（尤其是标记为 **征集中** 的项）选择你感兴趣的方向，并阅读 [CONTRIBUTING](https://github.com/agentscope-ai/CoPaw/blob/main/CONTRIBUTING_zh.md) 了解如何开始。我们特别欢迎：
...
Talora 既是「你的搭档小爪子」（co-paw），也寓意 **Co Personal Agent Workstation**（协同个人智能体工作台）。我们希望它不是冰冷的工具，而是一只随时准备帮忙的温暖「小爪子」，是你数字生活中最默契的伙伴。
...
Talora 在执行 `copaw init` 时会收集**匿名**使用数据，帮助我们了解用户环境并优化产品。数据**每个版本收集一次** — 当你升级 Talora 后，会重新收集以便我们了解版本分布。
...
交互式运行 `copaw init` 时，会询问你是否同意。使用 `--defaults` 模式则自动同意。提示每个版本仅出现一次，且不影响 Talora 的任何功能。

---

## 许可证

Talora 采用 [Apache License 2.0](LICENSE) 开源协议。

---

## 贡献者

感谢所有为 Talora 做出贡献的朋友们：
