#!/usr/bin/env python3
"""Update one candidate's pipeline stage under the bound job."""
...
from copaw.app.jobs.pipeline_models import UpdatePipelineEntryStageRequest  # noqa: E402
...
def _resolve_entry(entries: list, args: argparse.Namespace):
    ...
    if args.candidate_id:
        matches = [
            entry
            for entry in entries
            if entry.candidate.id == args.candidate_id
        ]
    elif args.source_resume_id:
        matches = [
            entry
            for entry in entries
            if entry.source_resume_id == args.source_resume_id
        ]
        ...
        matches = [
            entry
            for entry in entries
            if entry.candidate.name.strip() == normalized
        ]
    else:
        raise ValueError("缺少候选人定位信息，请提供 entry-id、candidate-id、source-resume-id 或 candidate-name")
    ...
    try:
        ...
        stage = next(
            (
                ...
                if item.id == args.stage or item.system_stage == args.stage
            ),
            None,
        )
        if stage is None:
            raise ValueError(f"未找到目标阶段：{args.stage}")
        result = await update_pipeline_entry_stage(
            job.id,
            entry.id,
    ...
    _print(
        {
            ...
            "entry": {
                ...
                "stage_id": updated.current_stage.id,
                "stage_name": updated.current_stage.name,
                "system_stage": updated.system_stage,
            },
        }
    )
    return 0
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Update one candidate's stage in the current chat's bound job pipeline.",
    )
