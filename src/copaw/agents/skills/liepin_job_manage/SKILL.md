...
description: "猎聘企业版职位管理。适用于查看企业版职位列表、拉取职位详情、创建企业版职位，以及把猎聘企业版职位真实绑定到当前 CoPaw 职位。查询、创建和绑定都必须走本 skill 提供的脚本，不要只做口头确认。"
metadata:
  {
...
- 创建企业版职位如果是基于当前 CoPaw 职位进行发布，创建成功后应继续拉详情并调用绑定脚本，把新的 `ejobId` 真实关联回来。
...
1. 如果当前 chat 还没绑定 CoPaw 职位，但用户目标是先建 CoPaw 职位，先用 `job_creator`
2. 如果用户要发布新的企业版职位，先完成字段收资，再用本 skill 的创建脚本
3. 如果创建成功且当前 chat 已绑定 CoPaw 职位，继续拉新职位详情并调用绑定脚本，把新 `ejobId` 关联回来
4. 如果用户是基于某个列表里的职位创建 CoPaw 职位，创建后立刻用该项的 `copawExternalJobId` 补绑外部关系，不要只创建本地职位
5. 如果列表里已经有真实 `ejobId`，继续拉详情并回写真实关系和真实 URL
6. 向用户反馈：查到了什么、创建了什么、绑定到了哪个 CoPaw 职位、是否已可在详情页看到
