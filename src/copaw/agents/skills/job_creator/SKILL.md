---
name: job_creator
description: "职位创建与职位绑定。适用于用户想先建一个职位框架、在当前 chat 里创建职位、或把当前 chat 绑定到明确已有职位的场景。只有当前 chat 尚未绑定职位时才允许使用；若已绑定职位，必须拒绝再次创建或改绑。"
metadata:
  {
...
2. 如果当前 chat 已经绑定职位，**禁止**在这个 chat 里再次创建职位，也禁止改绑到别的职位。
...
如果脚本返回“当前对话已绑定职位”，就不要继续创建，并直接告诉用户：
